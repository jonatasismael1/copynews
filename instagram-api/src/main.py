...
def collection_runs(limit: int = Query(20, ge=1, le=100), db: Session = Depends(get_db)):
    ...
    return [{"id": x.id, "status": x.status, "trigger": x.trigger, "apify_run_id": x.apify_run_id, "profiles": x.profiles, "profiles_succeeded": x.profiles_succeeded, "profiles_failed": x.profiles_failed, "posts_received": x.posts_received, "posts_found": x.posts_found, "posts_new": x.posts_new, "posts_updated": x.posts_updated, "collaborations_found": x.collaborations_found, "collaborations_made": x.collaborations_made, "collaborations_received": x.collaborations_received, "views_monitored": x.views_monitored, "reels_count": x.reels_count, "posts_count": x.posts_count, "carousels_count": x.carousels_count, "notification_status": x.notification_status, "notification_sent_at": x.notification_sent_at, "notification_error": x.notification_error, "error": x.error, "started_at": x.started_at, "finished_at": x.finished_at} for x in rows]
