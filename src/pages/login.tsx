  ...
  async function submit(v: Values) {
    if (forgot) {
      const { error } = await supabase.auth.resetPasswordForEmail(v.email, {
        redirectTo: `${location.origin}/configuracoes`,
      });
    ...
    <main className="grid min-h-dvh lg:grid-cols-[1.05fr_.95fr]">
      <section className="relative hidden overflow-hidden bg-sidebar p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-xl font-bold">Copy News</span>
        </div>
        <div className="relative max-w-xl">
          <p className="mb-5 text-xs font-semibold uppercase tracking-[.25em] text-emerald-300">
            Produção editorial inteligente
          </p>
          <h1 className="font-display text-5xl font-semibold leading-[1.08] tracking-tight">
            Da fonte à publicação, sem perder o fio da notícia.
          </h1>
          <p className="mt-6 max-w-lg text-lg leading-relaxed text-slate-300">
            Capture, transforme, revise e acompanhe todo o fluxo editorial em um
            só lugar.
          </p>
        </div>
        <p className="text-sm text-slate-500">Uso interno • Acesso protegido</p>
      </section>
      <section className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md">
          ...
          <h2 className="mt-3 font-display text-3xl font-bold tracking-tight">
            {forgot ? "Recuperar acesso" : "Entre na sua central"}
          </h2>
