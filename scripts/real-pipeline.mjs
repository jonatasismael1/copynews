...
try {
  const sourceUrl =
    process.env.PIPELINE_SOURCE_URL ||
    "https://www.youtube.com/watch?v=jNQXAC9IVRw";
  const queued = await client.functions.invoke("process-source-url", {
    body: {
      source_url: sourceUrl,
      editorial_tone: "Jornalístico, claro e direto",
    },
  });
  if (queued.error) throw queued.error;
  newsId = queued.data.news_item_id;
  ...
  while (Date.now() < deadline) {
    ...
    const job = await admin
      ...
      .select("*")
      .eq("id", queued.data.job_id)
      .single();
    if (job.error) throw job.error;
    if (job.data.status === "failed")
      throw new Error(
        `Pipeline failed at ${job.data.current_step}: ${job.data.error_code} ${job.data.error_message}`,
      );
  ...
  if (mediaPath)
    await admin.storage.from("temporary-media").remove([mediaPath]);
