...
async function enqueue(ctx: Awaited<ReturnType<typeof context>>, body: Record<string, unknown>) {
  ...
  const steps = { link: { status: "pending" }, media: [], title_label: { status: "pending" }, title_content: { status: "pending" }, caption_label: { status: "pending" }, caption_content: { status: "pending" } };
  const { data, error } = await ctx.admin.from("news_send_history").insert({ organization_id: ctx.profile.organization_id, news_id: newsId, recipient_id: recipientId, source_url: news.source_url, news_title: news.original_title, recipient_name: recipient.name, recipient_vehicle: recipient.vehicle, recipient_phone: phone, status: "queued", steps, queued_at: new Date().toISOString(), created_by: ctx.user.id }).select("id,status,queued_at").single();
